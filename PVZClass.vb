...
#Region "声明"
    ...
    'NEW 10.12 写任意类型内存
    Public Declare Function WriteProcessMemory Lib "kernel32.dll" Alias "WriteProcessMemory" _
        (ByVal hProcess As IntPtr,
         ByVal lpBaseAddress As IntPtr,
         ByRef lpBuffer As IntPtr,
         ByVal nSize As UInteger,
         ByRef lpNumberOfBytesWritten As IntPtr) _
    As Boolean
...
#Region "伪汇编标识符"
    ...
    Private Const mov_edx As Byte = &HBA
    Private Const mov_al As Byte = &HB0
...
#Region "逻辑实现方法"
    ...
    Public Shared Sub InitFunctions()
        If Not Game?.HasExited Then
            ...
            '改写
            Memory.WriteInteger(&H427072, Variable + 200 - 5 - &H427071)
            Memory.WriteBytes(&H42706C, {&HB9, 4, 0, 0, 0, &HE9})
            Memory.WriteInteger(Variable + 211, &H427076 - 5 - (Variable + 210))
            Memory.WriteBytes(Variable + 200, {&H89, &H48, &H14, &HC7, &H40, &H1C, &H78, &H4B, 5, 0, &HE9})
...
#End Region
        ...
        'NEW 10.12
#Region "写任意内存"
        Public Shared Function WriteMemory(Of T)(ByVal address As Integer, ByVal value As T) As Boolean
            Dim hProcess As IntPtr = Process.GetCurrentProcess().Handle
            Dim buffer As IntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(Of T)())
            Marshal.StructureToPtr(value, buffer, False)

            Dim size As UInteger = CUInt(Marshal.SizeOf(Of T)())
            Dim bytesWritten As IntPtr = IntPtr.Zero

            Dim result As Boolean = WriteProcessMemory(hProcess, New IntPtr(address), buffer, size, bytesWritten)
...
#Region "全局方法"
    ...
    Public Shared Sub QuickSave()
        ...
        Dim Asmcode As Byte() = {
            ...
            call­, 2, 0, 0, 0,
            jmp, 6,
            pushdw, &H30, &H8C, &H40, 0,
            ret,
            popad,
            ret
        }
        ReplaceBytes(Asmcode, 2, BaseAddress)
        Memory.Execute(Asmcode)
    End Sub

    '超级速度控制
    Public Shared Function SpeedCtrl_Init()
        Dim TmpAddress = Memory.AllocMemory()

        Dim jmpcode_Speedrun As Byte() = {
            mov_eax, 0, 0, 0, 0,
            &HFF, &HE0, &H90, &H90
        }

        Dim jmpcode_coin As Byte() = {
            mov_edx, 0, 0, 0, 0,
            &HFF, &HE2, &H90, &H90, &H90
        }

        Dim Asmcode As Byte() = {
        &H60, &HB8, &H1, &H0, &H0, &H0, &HBB, &H0, &H0, &H0,
        &H0, &H83, &HC3, &H1, &H39, &HC3, &H7D, &H22, &H60, &H8B,
        &H4E, &H4, &HC7, &H5, &H4E, &H0, &HEE, &H0, &H1, &H0,
        &H0, &H0, &HB8, &H20, &H59, &H41, &H0, &HFF, &HD0, &H61,
        &HC7, &H5, &H4E, &H0, &HEE, &H0, &H0, &H0, &H0, &H0,
        &HEB, &HD7, &H61, &H83, &HBA, &HF8, &H7, &H0, &H0, &H13,
        &H75, &H7, &HB8, &HBF, &H48, &H42, &H0, &HFF, &HE0, &HB8,
        &HC5, &H48, &H42, &H0, &HFF, &HE0, &HCC, &HCC, &H0, &H0,
        &H0, &H0, &HCC, &HCC, &H83, &H3D, &H4E, &H0, &HEE, &H0,
        &H1, &H75, &H1, &HC3, &H83, &HEC, &H20, &H53, &H8B, &HD8,
        &H83, &H43, &H4C, &H1, &HBA, &HA, &H15, &H43, &H0, &HFF,
        &HE2, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0, &H0
        }

        ReplaceBytes(jmpcode_Speedrun, 1, TmpAddress) '替换为申请的内存
        ReplaceBytes(jmpcode_coin, 1, TmpAddress + &H54) '替换为申请的内存地址 + 0x54的位置
        ReplaceBytes(Asmcode, &H18, TmpAddress + &H4E)
        ReplaceBytes(Asmcode, &H2A, TmpAddress + &H4E)
        ReplaceBytes(Asmcode, &H56, TmpAddress + &H4E)
        Memory.WriteByteArray(&H4248B6, jmpcode_Speedrun)
        Memory.WriteByteArray(&H431500, jmpcode_coin)
        Memory.WriteByteArray(TmpAddress, Asmcode)
        Return TmpAddress + &H2
    End Function


    '超级读档初始化
    Public Shared Sub Load_Init()
        Dim TmpAddress = Memory.AllocMemory()

        ''记录标志清空
        Dim mov_zero As Byte() = {
            0, 0, 0, 0, 0, 0, 0, 0
        }
        ...
        Dim jmpcode As Byte() = {
            mov_edi, 0, 0, 0, 0,
            &HFF, &HE7, &H90, &H90, &H90, &H90, &H90
        }

        ''读档CALL嵌入内容
        ' mov [ebp+00005558],ecx
        ' mov edx,[eax+00000320]
        ' cmp dword ptr [0058E282],01
        ' jne 0040032F
        ' mov [0058E282],00000000
        ' pushad 
        ' mov eax,0044F7A0
        ' mov esi,[006A9EC0]
        ' push esi
        ...
        Dim Asmcode As Byte() = {
            &H89, &H8D, &H58, &H55,
            &H0, &H0, &H8B, &H90, &H20, &H3, &H0, &H0,
            &H83, &H3D, &H82, &HE2, &H58, &H0, &H1, &H75,
            &H1A, &HC7, &H5, &H82, &HE2, &H58, &H0, &H0,
            &H0, &H0, &H0, &H60, &HB8, &HA0, &HF7, &H44,
            &H0, &H8B, &H35, &HC0, &H9E, &H6A, &H0, &H56,
            &HFF, &HD0, &H61, &HBF, &H58, &H60, &H41, &H0,
            &HFF, &HE7, &HCC, &HCC, &H0, &H0, &H0, &H0
        }

        ReplaceBytes(jmpcode, 1, TmpAddress)
        Memory.WriteByteArray(&H58E282, mov_zero)
        Memory.WriteByteArray(&H41604C, jmpcode)
        Memory.WriteByteArray(TmpAddress, Asmcode)
    End Sub
    ...
    Public Shared Sub QuickLoad()

        Dim Asmcode As Byte() = {
            pushad,
            mov_eax, &HA0, &HF7, &H44, 0,
            &H8B, &H35, &HC0, &H9E, &H6A, 0,
            &H83, &HBE, &H68, &H7, 0, 0, 0,
            &H74, &H3, &H56,
            &HFF, &HD0,
            popad,
            ret
    ...
    Public Shared Sub QuickSaveResumeAudio(ByVal IsPause As Boolean)
        ...
        Dim Asmcode As Byte() = {
           ...
           call­, 2, 0, 0, 0,
           jmp, 6,
           pushdw, &HA0, &H27, &H41, 0,
           ret,
           popad,
           ret
        }
        ReplaceBytes(Asmcode, 4, BaseAddress)
        Memory.Execute(Asmcode)
    ...
    Public Shared Sub CreateRake(ByVal Row As Byte, ByVal Column As Byte)
        If MainObjectExist Then
            ...
            'PVZ Class 方案 未解决图层问题
            'ReplaceBytes(AsmCode, 5, (BaseAddress))
            'Memory.WriteMemory(Of UShort)(&H40B9E3, &HA681)
            'Memory.WriteByte(&H40BB2B, 0)
            'Memory.WriteInteger(&H40BB3B, -1878241909)
            'Memory.WriteInteger(&H40BB41, -1877981813)
            'Memory.Execute(AsmCode)
            'Memory.WriteShort(&H40B9E3, &H7984)
            'Memory.WriteByte(&H40BB2B, &HFF)
            'Memory.WriteInteger(&H40BB41, 337921163)
            'Memory.WriteInteger(&H40BB3B, 270814347)
        End If
...
#Region "全局属性"
    ...
    Public Shared ReadOnly Property GriditemsCount As Integer
        ...
        End Get
    End Property
    Public Shared ReadOnly Property AllCards As Array
        Get
            Dim MaxIndex = PVZ.CardSlot.CardNum - 1
            Dim re(MaxIndex)
            Dim Num As Integer = 0
            For i = 0 To MaxIndex
                re(Num) = PVZ.CardSlot.GetCard(i)
                Num += 1
            Next
            ReDim Preserve re(Num - 1)
            Return re
...
#Region "MainObject对象"
    ...
    ''' 玉米炮冷却
    ''' </summary>
    ''' <param name="CobCD"></param>
    Public Shared Sub SetCobColdDownTime(CobCD As Int32)
        PVZ.Memory.WriteInteger(&H464D4D, CobCD)
    End Sub
    ''' <summary>
    ''' 植物对象
    ''' </summary>
    Public Class Plant
        ...
        ''' 计算植物图层
        ''' </summary>
        Public Function CalcLayer() As Integer
            Dim AsmCode As Byte() = {
                 &HBE, 0, 0, 0, 0,
                 call­, 2, 0, 0, 0,
                 jmp, 6,
                 pushdw, &HC0, &HE7, &H45, 0,
                 ret,
                 &HA3, 0, 0, 0, 0,
                ret
            }
...
#Region "MainObject属性"
    ...
    ''' 键控
    ''' </summary>
    Public Shared ReadOnly Property KeyAddress As Integer
        Get
            Return Memory.GetAddress(&H6A9EC0, &H320)
    ...
    Public Shared ReadOnly Property Base As Integer
        Get
            Return Memory.ReadInteger(&H6A9EC0)
        End Get
    End Property
    ''' <summary>
